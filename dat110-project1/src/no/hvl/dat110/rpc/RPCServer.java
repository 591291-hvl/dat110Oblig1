...
//				throw new UnsupportedOperationException(TODO.method());
			...
			byte[] requestbyte = requestmsg.getData();
			//find id of RPC method
			rpcid = requestmsg.getData()[0];
			//lookup the method
			RPCRemoteImpl RPCmethod = services.get(rpcid);
			
			
			byte[] bytes = new byte[requestbyte.length-1];
			for(int i = 0; i < requestbyte.length-1; i++) {
				bytes[i] = requestbyte[i+1];
			}
			//invoke method
			replymsg = new Message(RPCUtils.encapsulate(rpcid,RPCmethod.invoke(bytes)));
			//send back message
